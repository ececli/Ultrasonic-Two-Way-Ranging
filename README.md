...
* [twoWayRanging_Listener_SingleTone_v1.py](/twoWayRanging_Listener_SingleTone_v1.py) and [twoWayRanging_Master_SingleTone_v1.py](/twoWayRanging_Master_SingleTone_v1.py) are two main programs. They are very similar. The difference is that [twoWayRanging_Listener_SingleTone_v1.py](/twoWayRanging_Listener_SingleTone_v1.py) first listens the channel. After the peak is detected, it sends the signal out. Therefore, the device which runs [twoWayRanging_Listener_SingleTone_v1.py](/twoWayRanging_Listener_SingleTone_v1.py) gets information of T2 and T3. On the contrary, [twoWayRanging_Master_SingleTone_v1.py](/twoWayRanging_Master_SingleTone_v1.py) first sends the signal out and then listens the channel. The device which runs [twoWayRanging_Master_SingleTone_v1.py](/twoWayRanging_Master_SingleTone_v1.py) obtains T1 and T4. After this device receives T3-T2 information, this device (i.e., [twoWayRanging_Master_SingleTone_v1.py](/twoWayRanging_Master_SingleTone_v1.py)) estimates the distance between two devices. An important note: One device needs to run [twoWayRanging_Listener_SingleTone_v1.py](/twoWayRanging_Listener_SingleTone_v1.py) first. Then, the other device start to run   [twoWayRanging_Master_SingleTone_v1.py](/twoWayRanging_Master_SingleTone_v1.py).
